...
class Stats(object):
    ...
    def __init__(
        ...
        self._per_repo_stars: Optional[Dict[str, int]] = None
...
Languages:
  - {formatted_languages}"""
    ...
    @property
    async def per_repo_stars(self) -> Dict[str, int]:
        """
        :return: 每个仓库的 star 数
        """
        if self._per_repo_stars is not None:
            return self._per_repo_stars
        await self.get_stats()
        self._per_repo_stars = {}
        for repo in self._repos:
            repo_info = await self.queries.query_rest(f"/repos/{repo}")
            self._per_repo_stars[repo] = repo_info.get("stargazers_count", 0)
        return self._per_repo_stars
